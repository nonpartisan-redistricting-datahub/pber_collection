...
def shared_boundary_length(geom1, geom2):
    return geom1.boundary.intersection(geom2.boundary).length

def centroid_distance(geom1, geom2):
...
def district_splits_comb(level, splits_list, elections_gdf, district_gdf, unique_ID_col, district_gdf_ID, races_list,
                         elections_gdf_dist_ID, fill_level=2, remove_zero_vote_precs = False):
    '''
    ...
    need_splits = gp.GeoDataFrame(elections_gdf[elections_gdf[unique_ID_col].isin(splits_list)])
    others = gp.GeoDataFrame(elections_gdf[~elections_gdf[unique_ID_col].isin(splits_list)])
    ...
    if remove_zero_vote_precs:
        # Calculate new total votes column
        clean_votes["TOT_VOTES_new"] = clean_votes[races_list].sum(axis=1)

        # Isolate zero-vote precincts
        zero_votes = clean_votes[clean_votes["TOT_VOTES_new"] == 0].copy()

        for prec_id in zero_votes["UNIQUE_ID"]:
            zero_votes = clean_votes[clean_votes["TOT_VOTES_new"] == 0].copy()
            
            # Index and geometry of zero-vote row
            zero_idx = zero_votes[zero_votes["UNIQUE_ID"] == prec_id].index[0]
            zero_geom = zero_votes.at[zero_idx, "geometry"]

            # Remove suffix to identify potential match group
            base_id = "-(".join(prec_id.split("-(")[:-1])

            # Find matching candidates with non-zero votes
            prospects = clean_votes[
                (clean_votes["UNIQUE_ID"].str.contains(base_id, regex=False, na=False)) &
                (clean_votes["TOT_VOTES_new"] > 0)
            ].copy()

            if prospects.empty:
                print(f"No merge candidates found for: {prec_id}")
                continue

            # First try based on shared boundary
            prospects["Intersect"] = prospects["geometry"].apply(lambda g: shared_boundary_length(g, zero_geom))
            best_match_idx = prospects["Intersect"].idxmax()

            if prospects.loc[best_match_idx, "Intersect"] == 0:
                # Fallback to centroid distance
                prospects["Intersect"] = prospects["geometry"].apply(lambda g: centroid_distance(g, zero_geom))
                best_match_idx = prospects["Intersect"].idxmin()
